...
export interface WagmiNativeSendTransactionExecutionResult {
...
export async function wagmiNativeSendTransaction(
  config: Config,
  options: MiniKitSendTransactionOptions,
): Promise<WagmiNativeSendTransactionExecutionResult> {
  ...
  const data: SendTransactionResult = {
    status: 'success',
    version: 2,
    userOpHash:
      typeof response === 'string' && isTransactionHash(response)
        ? response
        : String(response),
    from: address,
  ...
  return {
    executedWith: connector?.id === 'worldApp' ? 'minikit' : 'wagmi',
    data,
  };
}
