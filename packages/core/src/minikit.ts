...
export class MiniKit {
  ...
  > {
    const active = this.getActiveMiniKit();
    if (active !== this) {
      return active.attestation<TFallback>(options);
    }
    return attestation<TFallback>(options, this.getContext());
  }

  /**
   * Close the mini app
   */
  static closeMiniApp<TFallback = CloseMiniAppResult>(
    options: MiniKitCloseMiniAppOptions<TFallback> = {},
  ): Promise<CommandResultByVia<CloseMiniAppResult, TFallback, 'minikit'>> {
    const active = this.getActiveMiniKit();
    if (active !== this) {
      return active.closeMiniApp<TFallback>(options);
    }
    return closeMiniApp<TFallback>(options, this.getContext());
  }
