...
const mockSendMiniKitEvent = jest.fn();
jest.mock('commands/types', () => ({
  ...jest.requireActual('commands/types'),
  sendMiniKitEvent: mockSendMiniKitEvent,
}));
describe('Microphone Helper', () => {
  ...
  beforeEach(() => {
    ...
    MockMediaStreamTrack.prototype.stop = jest.fn(function () {
      this.readyState = 'ended';
    ...
    MediaStreamTrack.prototype.stop = jest.fn(function () {
      this.readyState = 'ended';
      setTimeout(() => this.dispatchEvent(new Event('ended')), 0);
    });
  });
  ...
  describe('setupMicrophone', () => {
    it('should not send microphone events for video-only streams', async () => {
      ...
      expect(mockSendMiniKitEvent).not.toHaveBeenCalledWith(
    ...
    it('should send microphone events for audio streams', async () => {
      ...
      expect(mockSendMiniKitEvent).toHaveBeenCalledWith({
        command: 'microphone-stream-started',
        version: 1,
        payload: {
    ...
    it('should handle streams with no tracks gracefully', async () => {
      ...
      global.navigator.mediaDevices.getUserMedia = jest.fn(async () => ({
        id: 'empty-stream-id',
        getTracks: () => [],
        getAudioTracks: () => [],
        getVideoTracks: () => [],
      ...
      expect(mockSendMiniKitEvent).not.toHaveBeenCalled();
      // Verify stream was returned successfully
      expect(stream).toBeDefined();
